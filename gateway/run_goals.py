...
class GatewayGoalsMixin:
    ...
    async def _run_post_turn_hooks(
        ...
        if not getattr(getattr(self, "config", None), "multiplex_profiles", False):
            return await self._run_post_turn_hooks_inner(
                agent_result=agent_result, source=source, is_internal=is_internal, event=event,
            )
    ...
    async def _loop_wakeup_fire_one(
        ...
        now: float,
        warned_no_route: set,
        warned_foreign_scope: set | None = None,
        ...
        if multiplex and row_profile != scan_profile:
            warnings = warned_foreign_scope if warned_foreign_scope is not None else set()
            warning_key = (sid, row_profile or "unowned", scan_profile)
            if warning_key not in warnings:
                warnings.add(warning_key)
                logger.warning(
                    "loop wakeup: ignoring foreign or unowned profile row for session %s "
                    "(row profile=%s, store profile=%s)",
                    sid, row_profile or "unowned", scan_profile,
                )
            return
        ...
        profile = route_profile or scan_profile
        event_stub = {
            "session_key": "",
            ...
            **{
                key: route.get(key, "")
                for key in (
                    "chat_type", "thread_id", "user_id", "user_name",
                    "scope_id", "guild_id", "parent_chat_id",
                )
            },
            "profile": None if profile == "default" else profile,
        }
        source = self._build_process_event_source(event_stub)
        if source is None:
            return
        session_key = None
        with suppress(Exception):
            session_key = self._session_key_for_source(source)
        if session_key:
            # The persisted origin contains richer routing than legacy loop
            # metadata (guild/parent/profile) and is the durable authority.
            event_stub["session_key"] = session_key
            persisted_source = self._build_process_event_source(event_stub)
        ...
        if adapter is None:
            ...
            # Persisted transport provenance wins over the routed runtime. With
            # no provenance, use the shared resolver: it admits served satellites
            # but never borrows the primary bot for a failed dedicated adapter.
            adapters = self._adapters_for_profile(adapter_profile or profile)
        ...
        # Synthetic sources bypass adapter.build_source(); pin the selected
        # live transport, including shared-adapter profile routes.
        ...
        if session_key:
            ...
            try:
                ...
                pass
            try:
                owner_task = getattr(adapter, "_session_tasks", {}).get(session_key)
            except (TypeError, AttributeError):
                owner_task = None
        ...
        if mgr.state is not None and mgr.state.awaiting_response:
            # Reclaim only after durable route reconstruction and proof that
            # no live/queued owner remains. The previous tick did run.
            if not session_key:
                return
            released = await self._run_in_executor_with_context(mgr.release_stale_tick_claim, now)
            if not released:
                return
            logger.info("loop wakeup: released stale tick claim for session %s", sid)
    ...
    async def _loop_wakeup_watcher(self, interval: float = 15.0) -> None:
        ...
        async def _scan_one_store(profile_name: Optional[str]) -> None:
            ...
            for sid, state in active_loops:
                await self._loop_wakeup_fire_one(
                    sid, state, now, warned_no_route,
                    warned_foreign_scope=warned_foreign_scope, profile_name=profile_name,
                )
