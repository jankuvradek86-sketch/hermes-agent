...
class GatewayNotificationsMixin:
    ...
    def _build_process_event_source(self, evt: dict):
        ...
        # A parsed session namespace owns the profile; route-only loop events
        # carry their store's profile explicitly before a session key exists.
        profile = derived.get("profile") if derived else str(evt.get("profile") or "").strip() or None
        ...
        scope_id = _opt("scope_id") or _opt("guild_id")
