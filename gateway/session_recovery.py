...
class SessionRecoveryMixin:
    ...
    def _finish_route_transition(
        ...
        if self._db_for_key(session_key) and create_kwargs:
            created = self._create_session_row(
            ...
            if created:
                predecessor_id = create_kwargs.get("parent_session_id") or end_session_id
                self._migrate_loop_after_rotation(
                    session_key,
                    predecessor_id,
                    create_kwargs.get("session_id"),
                    reason=end_reason if end_session_id else "session_rotation",
                )
    ...
    def _create_session_row(self, session_key, db_create_kwargs, origin, display_name, *, log) -> bool:
        ...
        *,
        ...
        try:
            ...
            if session_db is None:
                return
            if self._named_profile_for_key(session_key) is not None:
